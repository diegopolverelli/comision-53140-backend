...
export const initPassport=()=>{
    ...
    passport.use(
        ...
        new github.Strategy(
            {
                clientID:"Iv23liSb8FDFykCpKyXU",
                clientSecret:"65e9d6f71ec71e3ee55f9b287712b8d0c64635c5",
        ...
        new local.Strategy(
            ...
            async(req, username, password, done)=>{
                try {
                    let {nombre} = req.body
                    if(!nombre){
                        console.log(`Registro - falta prop. name`)
                        return done(null, false)
                    }

                    let existe=await usuariosManager.getBy({email:username})
                    if(existe){
                        console.log(`Registro - el usuario ${username} ya existe en BD`)
                        return done(null, false)
                    }
            ...
            async(username, password, done)=>{
                try {
                    ...
                    if(!validaPassword(password, usuario.password)){
                        console.log(`Login - credenciales invalidas`)
                        return done(null, false)
                    }
